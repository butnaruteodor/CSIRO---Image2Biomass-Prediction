...
def train_model(train_idx, val_idx, embed_dir, seed, fold_idx, protocol_name):
    ...
    Returns (best_metrics, per_epoch_metrics) where per_epoch_metrics is a list
    of dicts with 'epoch' and 'weighted_r2' for each epoch.
    ...
    patience_counter = 0
    per_epoch_metrics = []  # Track weighted_r2 for every epoch
    ...
    return best_metrics, per_epoch_metrics
...
def run_experiment():
    ...
    # Store all results
    all_results = {}
    # Per-epoch metrics for convergence curves (saved separately for backward compatibility)
    all_per_epoch = {}
    ...
    for protocol_name, split_fn in SPLIT_STRATEGIES.items():
        ...
        # Per-epoch tracking for this protocol
        protocol_per_epoch = []
        all_per_epoch[protocol_name] = protocol_per_epoch
        ...
        for seed_idx, seed in enumerate(SEEDS):
            ...
            for fold_idx, (train_idx, val_idx) in enumerate(splits):
                ...
                metrics, per_epoch_metrics = train_model(train_idx, val_idx, EMBED_DIR, seed, fold_idx, protocol_name)
                metrics['fold'] = fold_idx
                metrics['seed'] = seed
                metrics['n_train'] = len(train_idx)
                metrics['n_val'] = len(val_idx)
                seed_fold_results.append(metrics)
                # Store per-epoch data with identifiers
                for pem in per_epoch_metrics:
                    protocol_per_epoch.append({
                        'protocol': protocol_name,
                        'seed': seed,
                        'fold': fold_idx,
                        'epoch': pem['epoch'],
                        'weighted_r2': pem['weighted_r2'],
                    })
    ...
    # Save per-epoch metrics separately (new file, does not affect backward compat)
    if all_per_epoch:
        per_epoch_path = os.path.join(RESULTS_DIR, 'per_epoch_metrics.pt')
        torch.save(all_per_epoch, per_epoch_path)
        print(f"Per-epoch metrics saved to {per_epoch_path}")
...
def generate_table_8(all_results):
    ...
    """
    rows = []
    protocol_order = ['random_stratified', 'date_grouped', 'date_location_grouped', 'leave_one_period_out']
